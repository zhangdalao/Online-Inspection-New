...
class LoginTest(RunTest):
    ...
    @ddt.data(*a.get_data_by_api(fieldname, "H5Login"))
    def test_H5Login(self, value):
        self.apiName = (inspect.stack()[0][3])[5:]
        #  获取测试环境参数
        env = value[self.env_num]
        # 通过环境参数获得接口url
        ...
        # 调用接口发起请求
        self.res = self.start(self.project, self.isSkip_num, self.apiName_num, url, self.method_num, self.headers_num,
                         self.para_num,
