...
class AuthorityTest(RunTest):
    ...
    def test_alogin(self, value):
        ...
        self.result = self.start(self.project, self.isSkip_num, self.apiName_num, url, self.method_num, self.headers_num,
                                 ...
                                 value, verify=False)
        sss["userId1"] = str(sss["userId1"])

    @ddt.data(*a.get_data_by_api(fieldname, "aopratemajordomo"))
    def test_aopratemajordomo(self, value):
        self.desc = value[self.desc_num]
        self.apiName = (inspect.stack()[0][3])[5:]  # 表示读取列表中的第一个元素（字典元素)的第三个元素？？？？？但是第三个应该是 请求头啊
        env = value[self.env_num]
        uri = self.a.get_apiPath(self.fieldname, self.apiName)
        url = self.a.get_domains()[env] + uri  # a.get_domains是字典，因为有好几个环境，根据测试环境来获得域名，域名+uri就是访问地址
        str_sign_list = [str(sss["userId1"]), str(sss["token1"]), self.timestamp, value[self.method_num].upper(), uri]
        # print(str_sign_list)
        value.append(str_sign_list)
        # print(value)
        #sss["version"] = sss["versionName"][1:]
        ...
        estateId = '"estateId":(.+?)'
        projectlist = re.findall(estateId,self.result)
        self.assertAlmostEqual(len(projectlist),4)
        self.assertAlmostEqual(projectlist[1],test)'''