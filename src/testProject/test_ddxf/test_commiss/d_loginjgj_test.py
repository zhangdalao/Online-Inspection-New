...
class LoginTest(RunTest):
	...
	def test_Login(self, value):
		if sss["env"] == "prod":
			...
			sss["sms_code"] = get_smsCode("pre", 'https://jr.fangdd.com.cn/jgj/api/user/smgsend', 'post', json={"mobile":"18652980686","type":1})
		elif sss["env"] == "test":
		...
		sss["cookies3"] = requests.utils.dict_from_cookiejar(self.result.cookies)
		print(sss)
	...
	def test_Nfactoring(self, value):
		...
		env = value[self.env_num]
		# 通过环境参数获得接口url
		url = self.a.get_domains()["syb"][env] + self.a.get_apiPath(self.fieldname, self.apiName)
		# 调用接口发起请求
		self.result = self.start(self.project, self.isSkip_num, self.apiName_num, url, self.method_num,
								 ...
								 value, cookies=sss["cookies3"], verify=False)
		time.sleep(3)
