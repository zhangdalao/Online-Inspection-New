...
class EsfDetailTest(RunTest):
	...
	@ddt.data(*a.get_data_by_api(fieldname, "EsfBargain"))
	def test_EsfBargain(self, value):
		# 通过函数名获取apiName参数的值
	...
	@ddt.data(*a.get_data_by_api(fieldname, "EsfDetailSeeHouse"))
	def test_EsfDetailSeeHouse(self, value):
		# 通过函数名获取apiName参数的值
		self.apiName = (inspect.stack()[0][3])[5:]
		env = value[self.env_num]
		# 通过环境参数获得接口url
