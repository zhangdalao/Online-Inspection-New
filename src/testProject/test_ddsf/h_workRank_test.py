...
class WorkRankTest(TemplateTeseCase):
	"""拉新业绩模块"""
	...
	def test_PullNewAgentRank(self):
		"""拉新业绩排行"""
	...
	def test_ActivityAchievement(self):
		"""拉新经纪人信息"""
		self.apiName = (inspect.stack()[0][3])[5:]
		url = self.dt.get_url(self.fieldname, self.apiName)
		data = self.dt.get_body()
		try:
			result = self.s.post(url=url, json=data)
			res = result.json()
			self.assertEqual(len(res["data"]["activityDetailOutPut"]["effectOutPutMap"]),
			                 len(res["data"]["activityDetailOutPutList"]))
		except Exception as er:
			send_ding(self.dd_dt["robot_url"], self.dd_dt["mobile"], "%s模块下%s接口异常:" % (self.fieldname, self.apiName) +
			          str(er))