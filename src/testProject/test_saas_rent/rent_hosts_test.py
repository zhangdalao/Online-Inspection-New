...
class Rent_HostsTest(RunTest):
	...
	def test_01_update_Rent_Hosts(self, value):
		...
		# 通过环境参数获得接口url
		url = self.a.get_domains()[env] + self.a.get_apiPath(self.fieldname, self.apiName)+"{}".format(sss["ID4"])
		# 调用接口发起请求
		...
		try:
			...
			send_ding(robot_url, mobile, content=f"修改业主信息异常！接口返回为：{res}, 接口预期结果为：{self.expect}")
			raise err
