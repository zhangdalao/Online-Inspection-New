...
class Rent_InspectTest(RunTest):
    ...
    @ddt.data(*a.get_data_by_api(fieldname, "set_Relator"))  # 接口对应的名称
    def test_02_set_Relator(self, value):
        """修改房源相关人-开盘人"""
        # 通过函数名获取apiName参数的值
        self.apiName = (inspect.stack()[0][3])[8:]
        # 获取测试环境参数
        ...
        try:
            ...
            robot_url = json_dict["robot_url"]
            mobile = json_dict["mobile"]
            send_ding(robot_url, mobile, content=f"更新房源相关人异常！接口返回为：{res}, 接口预期结果为：{self.expect}")
            raise err
