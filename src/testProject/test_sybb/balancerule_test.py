...
class BalanceRuleTest(RunTest):
	...
	@ddt.data(*a.get_data_by_api(fieldname, "BalanceRuleList"))
	def test_BalanceRuleList(self, value):
		# 通过函数名获取apiName参数的值
	...
	def test_BalanceRuleEdit(self, value):
		# 通过函数名获取apiName参数的值
		self.apiName = (inspect.stack()[0][3])[5:]
		# 获取测试环境参数
		env = value[self.env_num]
