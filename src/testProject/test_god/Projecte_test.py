...
class ProjectTest(RunTest):
    ...
    def test_ProjectList(self, value):
        ...
        try:
            ...
            send_ding(robot_url, mobile, content=f"推广项目管理列表异常，接口返回为：{self.res}, 接口预期结果为：{self.expect}")
            raise err

    @ddt.data(*a.get_data_by_api(fieldname, "ProjecSearch"))
    def test_ProjecSearch(self, value):
        # 通过函数名获取apiName参数的值
        self.apiName = (inspect.stack()[0][3])[5:]
        # 获取测试环境参数
        env = value[self.env_num]
        ...
        try:
            ...
            robot_url = json_dict["robot_url"]
            mobile = json_dict["mobile"]
            send_ding(robot_url, mobile, content=f"推广项目搜索异常，接口返回为：{self.res}, 接口预期结果为：{self.expect}")
            raise err
    ...
    def test_ProjecEdit(self, value):
        ...
        try:
            ...
            robot_url = json_dict["robot_url"]
            mobile = json_dict["mobile"]
            send_ding(robot_url, mobile, content=f"推广项目编辑异常，接口返回为：{self.res}, 接口预期结果为：{self.expect}")
            raise err
