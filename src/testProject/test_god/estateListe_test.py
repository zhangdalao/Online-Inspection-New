...
class EestateTest(RunTest):
    ...
    def test_EstateList(self, value):
        ...
        try:
            ...
            send_ding(robot_url, mobile, content=f"god楼盘列表异常，接口返回为：{res}, 接口预期结果为：{self.expect}")
            raise err
    ...
    def test_EstateSearch(self, value):
        ...
        try:
            ...
            robot_url = json_dict["robot_url"]
            mobile = json_dict["mobile"]
            send_ding(robot_url, mobile, content=f"god楼盘搜索异常，接口返回为：{res}, 接口预期结果为：{self.expect}")
            raise err
