...
class RunMethod(MethodException):
	...
	def run_main(self, url=None, method=None, headers=None, data=None, **kw):
		...
		try:
			if method:
				if method.lower() in method_list1:
					...
					res = requests.request(method.lower(), url, headers=headers, **kw)
					...
					if "application/json" in str(headers).lower():
						...
						res = requests.request(method.lower(), url, data=data, headers=headers, **kw)
				else:
